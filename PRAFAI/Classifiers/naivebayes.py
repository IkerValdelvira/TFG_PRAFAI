...
class NaiveBayes(object):
    ...
    def train(train_dev, test, file_name, path):
        file = open(os.path.join(path, "REPORT_NaiveBayes_" + file_name + ".txt"), "w")
        ...
        # Save the model
        joblib.dump(classifier, os.path.join(path, "NB_MODEL_" + file_name + ".joblib"), compress=3)
        print('Trained model saved in: ' + os.path.join(path, os.path.join(path, "NB_MODEL_" + file_name + ".joblib")))
        ...
        print('Model evaluation report saved in: ' + os.path.join(path, "REPORT_NaiveBayes_" + file_name + ".txt"))
        ...
        plt.plot(fpr, tpr, color='red',
                 label='Naive Bayes (AUC = %0.2f)' % auc_metric)
