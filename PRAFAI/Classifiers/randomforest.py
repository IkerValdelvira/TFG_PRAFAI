...
class RandomForest(object):
    ...
    def train(train_dev, test, file_name, path):
        file = open(os.path.join(path, "REPORT_RandomForest_" + file_name + ".txt"), "w")
        ...
        file.write('Best min_samples_split:' + str(best_model.best_estimator_.get_params()['min_samples_split']) + "\n")
        file.write('Best min_samples_leaf:' + str(best_model.best_estimator_.get_params()['min_samples_leaf']) + "\n")
        ...
        # Save the model
        joblib.dump(best_estimator, os.path.join(path, "RF_MODEL_" + file_name + ".joblib"), compress=3)
        print('Trained model saved in: ' + os.path.join(path, os.path.join(path, "RF_MODEL_" + file_name + ".joblib")))
        ...
        fig, ax = plt.subplots()
        rf_roc = RandomForestClassifier(n_estimators=best_model.best_estimator_.get_params()['n_estimators'],
        ...
        for i, (train, test) in enumerate(cv.split(X_train, y_train)):
            rf_roc.fit(X_train.iloc[train], y_train.iloc[train])
            viz = plot_roc_curve(rf_roc, X_train.iloc[test], y_train.iloc[test],
        ...
        plt.plot(fpr, tpr, color='red',
                 label='Random Forest (AUC = %0.2f)' % auc_metric)
        ...
        print('Model evaluation report saved in: ' + os.path.join(path, "REPORT_RandomForest_" + file_name + ".txt"))
