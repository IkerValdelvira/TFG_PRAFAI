...
class Logistic(object):
    ...
    def train(train_dev, test, file_name, path):
        file = open(os.path.join(path, "REPORT_LogisticRegression_" + file_name + ".txt"), "w")
        ...
        y_pred_train = cross_val_predict(logmodel, X_train, y_train, cv=10)
        y_pred_test = logmodel.predict(X_test)

        # Save the model
        joblib.dump(logmodel, os.path.join(path, "LR_MODEL_" + file_name + ".joblib"), compress=3)
        print('Trained model saved in: ' + os.path.join(path, os.path.join(path, "LR_MODEL_" + file_name + ".joblib")))
        ...
        fig, ax = plt.subplots()
        logmodel_roc = LogisticRegression(max_iter=1500)
        for i, (train, test) in enumerate(cv.split(X_train, y_train)):
            logmodel_roc.fit(X_train.iloc[train], y_train.iloc[train])
            viz = plot_roc_curve(logmodel_roc, X_train.iloc[test], y_train.iloc[test],
        ...
        plt.plot(fpr, tpr, color='red',
                 label='Logistic Regression (AUC = %0.2f)' % auc_metric)
        ...
        print('Model evaluation report saved in: ' + os.path.join(path, "REPORT_LogisticRegression_" + file_name + ".txt"))

        return y_train, mean_fpr, mean_tpr, mean_auc, logmodel, X_test, y_test
    ...
    def evaluation_comparison(results, path):       # Por cada elemento en results obtenemos un array: [y_train, fpr, tpr, auc, classifier, X_test, y_test]
        ...
        if(len([val for val in results.keys() if 'StandardScaler' in val]) > 0):
            ...
            if (len(result_mean_SS) != 0):
                plt.plot(result_mean_SS[1], result_mean_SS[2], color='orange', label=algorithm_name + ', MV mean (AUC = %0.2f)' % result_mean_SS[3])
            if (len(result_med_SS) != 0):
                plt.plot(result_med_SS[1], result_med_SS[2], color='red', label=algorithm_name + ', MV median (AUC = %0.2f)' % result_med_SS[3])
            if (len(result_pred_SS) != 0):
                plt.plot(result_pred_SS[1], result_pred_SS[2], color='green', label=algorithm_name + ', MV predicted (AUC = %0.2f)' % result_pred_SS[3])
            if (len(result_diff_SS) != 0):
                plt.plot(result_diff_SS[1], result_diff_SS[2], color='purple', label=algorithm_name + ', MV different value (AUC = %0.2f)' % result_diff_SS[3])
            plt.plot(p_fpr, p_tpr, linestyle='--', color='blue')
            # title
            ...
            print("\n[StandardScaler] Validation set ROC curve comparison saved in: " + os.path.join(path, "StandardScaler", "ROCcurve(Train_Dev)_Comparison_StandardScaler.png"))
            plt.close()
            ...
            if (len(result_med_SS) != 0):
                ...
                fpr_median, tpr_median, thresh_median = roc_curve(result_pred_SS[6], y_pred_prob_median[:, 1], pos_label=1)
                auc_median = auc(fpr_median, tpr_median)
            if (len(result_pred_SS) != 0):
                y_pred_prob_predicted = result_pred_SS[4].predict_proba(result_pred_SS[5])
                fpr_predicted, tpr_predicted, thresh_predicted = roc_curve(result_pred_SS[6], y_pred_prob_predicted[:, 1], pos_label=1)
            ...
            # plot roc curves
            if (len(result_mean_SS) != 0):
                plt.plot(fpr_mean, tpr_mean, color='orange', label=algorithm_name + ', MV mean (AUC = %0.2f)' % auc_mean)
            if (len(result_med_SS) != 0):
                plt.plot(fpr_median, tpr_median, color='red', label=algorithm_name + ', MV median (AUC = %0.2f)' % auc_median)
            if (len(result_pred_SS) != 0):
                plt.plot(fpr_predicted, tpr_predicted, color='green', label=algorithm_name + ', MV predicted (AUC = %0.2f)' % auc_predicted)
            if (len(result_diff_SS) != 0):
                plt.plot(fpr_diff, tpr_diff, color='purple', label=algorithm_name + ', MV different value (AUC = %0.2f)' % auc_diff)
            plt.plot(p_fpr, p_tpr, linestyle='--', color='blue')
            # title
            ...
            print("\n[StandardScaler] Test set ROC curve comparison saved in: " + os.path.join(path, "StandardScaler", "Comparisons", "ROCcurve(Test)_Comparison_StandardScaler.png"))
            plt.close()
        ...
        if(len([val for val in results.keys() if 'MinMaxScaler' in val]) > 0):
            ...
            if (len(result_mean_MMS) != 0):
                plt.plot(result_mean_MMS[1], result_mean_MMS[2], color='orange', label=algorithm_name + ', MV mean (AUC = %0.2f)' % result_mean_MMS[3])
            if (len(result_med_MMS) != 0):
                plt.plot(result_med_MMS[1], result_med_MMS[2], color='red', label=algorithm_name + ', MV median (AUC = %0.2f)' % result_med_MMS[3])
            if (len(result_pred_MMS) != 0):
                plt.plot(result_pred_MMS[1], result_pred_MMS[2], color='green', label=algorithm_name + ', MV predicted (AUC = %0.2f)' % result_pred_MMS[3])
            if (len(result_diff_MMS) != 0):
                plt.plot(result_diff_MMS[1], result_diff_MMS[2], color='purple', label=algorithm_name + ', MV different value (AUC = %0.2f)' % result_diff_MMS[3])
            plt.plot(p_fpr, p_tpr, linestyle='--', color='blue')
            # title
            ...
            print("\n[MinMaxScaler] Validation set ROC curve comparison saved in: " + os.path.join(path, "MinMaxScaler", "Comparisons", "ROCcurve(Train_Dev)_Comparison_MinMaxScaler.png"))
            plt.close()
            ...
            if (len(result_med_MMS) != 0):
                ...
                fpr_median, tpr_median, thresh_median = roc_curve(result_med_MMS[6], y_pred_prob_median[:, 1], pos_label=1)
                auc_median = auc(fpr_median, tpr_median)
            if (len(result_pred_MMS) != 0):
                y_pred_prob_predicted = result_pred_MMS[4].predict_proba(result_pred_MMS[5])
                fpr_predicted, tpr_predicted, thresh_predicted = roc_curve(result_pred_MMS[6], y_pred_prob_predicted[:, 1], pos_label=1)
                auc_predicted = auc(fpr_predicted, tpr_predicted)
            if (len(result_diff_MMS) != 0):
            ...
            print("\n[MinMaxScaler] Test set ROC curve comparison saved in: " + os.path.join(path, "MinMaxScaler", "Comparisons", "ROCcurve(Test)_Comparison_MinMaxScaler.png"))
            plt.close()
        ...
        if(len([val for val in results.keys() if 'MaxAbsScaler' in val]) > 0):
            ...
            if (len(result_mean_MAS) != 0):
                plt.plot(result_mean_MAS[1], result_mean_MAS[2], color='orange', label=algorithm_name + ', MV mean (AUC = %0.2f)' % result_mean_MAS[3])
            if (len(result_med_MAS) != 0):
                plt.plot(result_med_MAS[1], result_med_MAS[2], color='red', label=algorithm_name + ', MV median (AUC = %0.2f)' % result_med_MAS[3])
            if (len(result_pred_MAS) != 0):
                plt.plot(result_pred_MAS[1], result_pred_MAS[2], color='green', label=algorithm_name + ', MV predicted (AUC = %0.2f)' % result_pred_MAS[3])
            if (len(result_diff_MAS) != 0):
                plt.plot(result_diff_MAS[1], result_diff_MAS[2], color='purple', label=algorithm_name + ', MV different value (AUC = %0.2f)' % result_diff_MAS[3])
            plt.plot(p_fpr, p_tpr, linestyle='--', color='blue')
            # title
            ...
            print("\n[MaxAbsScaler] Validation set ROC curve comparison saved in: " + os.path.join(path, "MaxAbsScaler", "Comparisons", "ROCcurve(Train_Dev)_Comparison_MaxAbsScaler.png"))
            plt.close()
            ...
            if (len(result_med_MAS) != 0):
                ...
                fpr_median, tpr_median, thresh_median = roc_curve(result_med_MAS[6], y_pred_prob_median[:, 1], pos_label=1)
                auc_median = auc(fpr_median, tpr_median)
            if (len(result_pred_MAS) != 0):
                y_pred_prob_predicted = result_pred_MAS[4].predict_proba(result_pred_MAS[5])
                fpr_predicted, tpr_predicted, thresh_predicted = roc_curve(result_pred_MAS[6], y_pred_prob_predicted[:, 1], pos_label=1)
                auc_predicted = auc(fpr_predicted, tpr_predicted)
            if (len(result_diff_MAS) != 0):
            ...
            print("\n[MaxAbsScaler] Test set ROC curve comparison saved in: " + os.path.join(path, "MaxAbsScaler", "Comparisons", "ROCcurve(Test)_Comparison_MaxAbsScaler.png"))
            plt.close()
        ...
        if(len([val for val in results.keys() if 'QuantileTransformer' in val]) > 0):
            ...
            if (len(result_mean_QT) != 0):
                plt.plot(result_mean_QT[1], result_mean_QT[2], color='orange', label=algorithm_name + ', MV mean (AUC = %0.2f)' % result_mean_QT[3])
            if (len(result_med_QT) != 0):
                plt.plot(result_med_QT[1], result_med_QT[2], color='red', label=algorithm_name + ', MV median (AUC = %0.2f)' % result_med_QT[3])
            if (len(result_pred_QT) != 0):
                plt.plot(result_pred_QT[1], result_pred_QT[2], color='green', label=algorithm_name + ', MV predicted (AUC = %0.2f)' % result_pred_QT[3])
            if (len(result_diff_QT) != 0):
                plt.plot(result_diff_QT[1], result_diff_QT[2], color='purple', label=algorithm_name + ', MV different value (AUC = %0.2f)' % result_diff_QT[3])
            plt.plot(p_fpr, p_tpr, linestyle='--', color='blue')
            # title
            ...
            print("\n[QuantileTransformer] Validation set ROC curve comparison saved in: " + os.path.join(path, "QuantileTransformer", "Comparisons", "ROCcurve(Train_Dev)_Comparison_QuantileTransformer.png"))
            plt.close()
            ...
            if (len(result_med_QT) != 0):
                ...
                fpr_median, tpr_median, thresh_median = roc_curve(result_med_QT[6], y_pred_prob_median[:, 1], pos_label=1)
                auc_median = auc(fpr_median, tpr_median)
            if (len(result_pred_QT) != 0):
                y_pred_prob_predicted = result_pred_QT[4].predict_proba(result_pred_QT[5])
                fpr_predicted, tpr_predicted, thresh_predicted = roc_curve(result_pred_QT[6], y_pred_prob_predicted[:, 1], pos_label=1)
                auc_predicted = auc(fpr_predicted, tpr_predicted)
            if (len(result_diff_QT) != 0):
            ...
            print("\n[QuantileTransformer] Test set ROC curve comparison saved in: " + os.path.join(path, "QuantileTransformer", "Comparisons", "ROCcurve(Test)_Comparison_QuantileTransformer.png"))
            plt.close()
        ...
        if(len([val for val in results.keys() if 'Normalizer' in val]) > 0):
            ...
            if (len(result_mean_N) != 0):
                plt.plot(result_mean_N[1], result_mean_N[2], color='orange', label=algorithm_name + ', MV mean (AUC = %0.2f)' % result_mean_N[3])
            if (len(result_med_N) != 0):
                plt.plot(result_med_N[1], result_med_N[2], color='red', label=algorithm_name + ', MV median (AUC = %0.2f)' % result_med_N[3])
            if (len(result_pred_N) != 0):
                plt.plot(result_pred_N[1], result_pred_N[2], color='green', label=algorithm_name + ', MV predicted (AUC = %0.2f)' % result_pred_N[3])
            if (len(result_diff_N) != 0):
                plt.plot(result_diff_N[1], result_diff_N[2], color='purple', label=algorithm_name + ', MV different value (AUC = %0.2f)' % result_diff_N[3])
            plt.plot(p_fpr, p_tpr, linestyle='--', color='blue')
            # title
            ...
            print("\n[Normalizer] Validation set ROC curve comparison saved in: " + os.path.join(path, "Normalizer", "Comparisons", "ROCcurve(Train_Dev)_Comparison_Normalizer.png"))
            plt.close()
            ...
            if (len(result_med_N) != 0):
                ...
                fpr_median, tpr_median, thresh_median = roc_curve(result_med_N[6], y_pred_prob_median[:, 1], pos_label=1)
                auc_median = auc(fpr_median, tpr_median)
            if (len(result_pred_N) != 0):
                y_pred_prob_predicted = result_pred_N[4].predict_proba(result_pred_N[5])
                fpr_predicted, tpr_predicted, thresh_predicted = roc_curve(result_pred_N[6], y_pred_prob_predicted[:, 1], pos_label=1)
                auc_predicted = auc(fpr_predicted, tpr_predicted)
            if (len(result_diff_N) != 0):
            ...
            print("\n[Normalizer] Test set ROC curve comparison saved in: " + os.path.join(path, "Normalizer", "Comparisons", "ROCcurve(Test)_Comparison_Normalizer.png"))
            plt.close()
