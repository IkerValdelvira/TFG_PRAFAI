...
class XGBoost(object):
    ...
    def train(train_dev, test, file_name, path):
        file = open(os.path.join(path, "REPORT_XGBoost_" + file_name + ".txt"), "w")
        ...
        # Create new XGBClassifier object
        classifier = XGBClassifier(use_label_encoder=False, eval_metric='logloss')
        # Use GridSearch
        clf = GridSearchCV(classifier, hyperparameters, cv=5)
        # Fit the model
        best_model = clf.fit(X_train, y_train)
        # Print The value of best Hyperparameters
        file.write('Best n_estimators:' + str(best_model.best_estimator_.get_params()['n_estimators']) + "\n")
        file.write('Best max_depth:' + str(best_model.best_estimator_.get_params()['max_depth']) + "\n")
        file.write('Best learning_rate:' + str(best_model.best_estimator_.get_params()['learning_rate']) + "\n")
        file.write('Best subsample:' + str(best_model.best_estimator_.get_params()['subsample']) + "\n")
        file.write('Best colsample_bytree:' + str(best_model.best_estimator_.get_params()['colsample_bytree']) + "\n")
        file.write('Best gamma:' + str(best_model.best_estimator_.get_params()['gamma']) + "\n")

        # Model and predictions
        best_estimator = best_model.best_estimator_
        y_pred_train = cross_val_predict(best_estimator, X_train, y_train, cv=5)
        y_pred_test = best_estimator.predict(X_test)

        # Save the model
        joblib.dump(best_estimator, os.path.join(path, "XGB_MODEL_" + file_name + ".joblib"), compress=3)
        print('Trained model saved in: ' + os.path.join(path, os.path.join(path, "XGB_MODEL_" + file_name + ".joblib")))
        ...
        plt.plot(fpr, tpr, color='red',
                 label='XGBoost (AUC = %0.2f)' % auc_metric)
        ...
        print('Model evaluation report saved in: ' + os.path.join(path, "REPORT_XGBoost_" + file_name + ".txt"))

        return y_train, mean_fpr, mean_tpr, mean_auc, best_estimator, X_test, y_test
    ...
    def evaluation_comparison(results, path):  # Por cada elemento en results obtenemos un array: [y_train, fpr, tpr, auc, classifier, X_test, y_test]
        ...
        if (len([val for val in results.keys() if 'StandardScaler' in val]) > 0):
            ...
            for val in results.keys():
                if '_mv_StandardScaler' in val:
                    result_mv_SS = results[val]
            for val in results.keys():
            ...
            # plot roc curves
            if (len(result_mv_SS) != 0):
                plt.plot(result_mv_SS[1], result_mv_SS[2], color='yellow',
                         label=algorithm_name + ', Missing Values (AUC = %0.2f)' % result_mv_SS[3])
            ...
            # roc curve for models
            if (len(result_mv_SS) != 0):
                y_pred_prob_mv = result_mv_SS[4].predict_proba(result_mv_SS[5])
                fpr_mv, tpr_mv, thresh_mv = roc_curve(result_mv_SS[6], y_pred_prob_mv[:, 1], pos_label=1)
            ...
            # plot roc curves
            if (len(result_mv_SS) != 0):
                plt.plot(fpr_mv, tpr_mv, color='yellow', label=algorithm_name + ', Missing Values (AUC = %0.2f)' % auc_mv)
            if (len(result_mean_SS) != 0):
        ...
        if (len([val for val in results.keys() if 'MinMaxScaler' in val]) > 0):
            ...
            result_diff_MMS = []
            for val in results.keys():
                if '_mv_MinMaxScaler' in val:
                    result_mv_MMS = results[val]
            for val in results.keys():
            ...
            # plot roc curves
            if (len(result_mv_MMS) != 0):
                plt.plot(result_mv_MMS[1], result_mv_MMS[2], color='yellow',
                         label=algorithm_name + ', Missing Values (AUC = %0.2f)' % result_mv_MMS[3])
            ...
            # roc curve for models
            if (len(result_mv_MMS) != 0):
                y_pred_prob_mv = result_mv_MMS[4].predict_proba(result_mv_MMS[5])
                fpr_mv, tpr_mv, thresh_mv = roc_curve(result_mv_MMS[6], y_pred_prob_mv[:, 1], pos_label=1)
                auc_mv = auc(fpr_mv, tpr_mv)
            if (len(result_mean_MMS) != 0):
        ...
        if (len([val for val in results.keys() if 'MaxAbsScaler' in val]) > 0):
            ...
            result_diff_MAS = []
            for val in results.keys():
                if '_mv_MaxAbsScaler' in val:
                    result_mv_MAS = results[val]
            for val in results.keys():
            ...
            # plot roc curves
            if (len(result_mv_MAS) != 0):
                plt.plot(result_mv_MAS[1], result_mv_MAS[2], color='yellow',
                         label=algorithm_name + ', Missing Values (AUC = %0.2f)' % result_mv_MAS[3])
            ...
            # roc curve for models
            if (len(result_mv_MAS) != 0):
                y_pred_prob_mv = result_mv_MAS[4].predict_proba(result_mv_MAS[5])
                fpr_mv, tpr_mv, thresh_mv = roc_curve(result_mv_MAS[6], y_pred_prob_mv[:, 1], pos_label=1)
                auc_mv = auc(fpr_mv, tpr_mv)
            if (len(result_mean_MAS) != 0):
        ...
        if (len([val for val in results.keys() if 'QuantileTransformer' in val]) > 0):
            ...
            result_diff_QT = []
            for val in results.keys():
                if '_mv_QuantileTransformer' in val:
                    result_mv_QT = results[val]
            for val in results.keys():
            ...
            # plot roc curves
            if (len(result_mv_QT) != 0):
                plt.plot(result_mv_QT[1], result_mv_QT[2], color='yellow',
                         label=algorithm_name + ', Missing Values (AUC = %0.2f)' % result_mv_QT[3])
            ...
            plt.legend(loc='best')
            plt.savefig(
                os.path.join(path, "QuantileTransformer", "Comparisons", "ROCcurve(Train_Dev)_Comparison_QuantileTransformer.png"))
            ...
            # roc curve for models
            if (len(result_mv_QT) != 0):
                y_pred_prob_mv = result_mv_QT[4].predict_proba(result_mv_QT[5])
                fpr_mv, tpr_mv, thresh_mv = roc_curve(result_mv_QT[6], y_pred_prob_mv[:, 1], pos_label=1)
                auc_mv = auc(fpr_mv, tpr_mv)
            if (len(result_mean_QT) != 0):
        ...
        if (len([val for val in results.keys() if 'Normalizer' in val]) > 0):
            ...
            result_diff_N = []
            for val in results.keys():
                if '_mv_Normalizer' in val:
                    result_mv_N = results[val]
            for val in results.keys():
            ...
            # plot roc curves
            if (len(result_mv_N) != 0):
                plt.plot(result_mv_N[1], result_mv_N[2], color='yellow',
                         label=algorithm_name + ', Missing Values (AUC = %0.2f)' % result_mv_N[3])
            ...
            # roc curve for models
            if (len(result_mv_N) != 0):
                y_pred_prob_mv = result_mv_N[4].predict_proba(result_mv_N[5])
                fpr_mv, tpr_mv, thresh_mv = roc_curve(result_mv_N[6], y_pred_prob_mv[:, 1], pos_label=1)
                auc_mv = auc(fpr_mv, tpr_mv)
            if (len(result_mean_N) != 0):
