...
class Bagging(object):
    ...
    def train(train_dev, test, file_name, path):
        file = open(os.path.join(path, "REPORT_Bagging_" + file_name + ".txt"), "w")
        ...
        # Create new Bagging object
        bagging_clf = BaggingClassifier(LogisticRegression(max_iter=1500))
        # Use GridSearch
        clf = GridSearchCV(bagging_clf, hyperparameters, cv=5)
        # Fit the model
        best_model = clf.fit(X_train, y_train)
        # Print The value of best Hyperparameters
        file.write('Best n_estimators:' + str(best_model.best_estimator_.get_params()['n_estimators']) + "\n")
        file.write('Best max_samples:' + str(best_model.best_estimator_.get_params()['max_samples']) + "\n")
        file.write('Best max_features:' + str(best_model.best_estimator_.get_params()['max_features']) + "\n")
        ...
        # Save the model
        joblib.dump(best_estimator, os.path.join(path, "B_MODEL_" + file_name + ".joblib"), compress=3)
        print('Trained model saved in: ' + os.path.join(path, os.path.join(path, "B_MODEL_" + file_name + ".joblib")))
        ...
        fig, ax = plt.subplots()
        bagging_clf_roc = BaggingClassifier(base_estimator=LogisticRegression(max_iter=1500),
                                            n_estimators=best_model.best_estimator_.get_params()['n_estimators'],
                                            max_samples=best_model.best_estimator_.get_params()['max_samples'],
                                            max_features=best_model.best_estimator_.get_params()['max_features'])
        for i, (train, test) in enumerate(cv.split(X_train, y_train)):
            bagging_clf_roc.fit(X_train.iloc[train], y_train.iloc[train])
            viz = plot_roc_curve(bagging_clf_roc, X_train.iloc[test], y_train.iloc[test],
        ...
        plt.plot(fpr, tpr, color='red',
                 label='Bagging (AUC = %0.2f)' % auc_metric)
        ...
        print('Model evaluation report saved in: ' + os.path.join(path, "REPORT_Bagging_" + file_name + ".txt"))
