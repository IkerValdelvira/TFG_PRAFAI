...
class AdaBoost(object):
    ...
    def train(train_dev, test, file_name, path):
        file = open(os.path.join(path, "REPORT_AdaBoost_" + file_name + ".txt"), "w")
        ...
        # Create new AdaBoost object
        adaboost_clf = AdaBoostClassifier(LogisticRegression(max_iter=1500))
        # Use GridSearch
        clf = GridSearchCV(adaboost_clf, hyperparameters, cv=5)
        # Fit the model
        ...
        # Save the model
        joblib.dump(best_estimator, os.path.join(path, "AB_MODEL_" + file_name + ".joblib"), compress=3)
        print('Trained model saved in: ' + os.path.join(path, os.path.join(path, "AB_MODEL_" + file_name + ".joblib")))
        ...
        fig, ax = plt.subplots()
        adaboost_clf_roc = AdaBoostClassifier(base_estimator=LogisticRegression(max_iter=1500),
        ...
        for i, (train, test) in enumerate(cv.split(X_train, y_train)):
            adaboost_clf_roc.fit(X_train.iloc[train], y_train.iloc[train])
            viz = plot_roc_curve(adaboost_clf_roc, X_train.iloc[test], y_train.iloc[test],
        ...
        plt.plot(fpr, tpr, color='red',
                 label='AdaBoost (AUC = %0.2f)' % auc_metric)
        ...
        print('Model evaluation report saved in: ' + os.path.join(path,"REPORT_AdaBoost_" + file_name + ".txt"))
