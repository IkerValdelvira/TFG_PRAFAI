...
class KNN(object):
    ...
    def train(train_dev, test, file_name, path):
        file = open(os.path.join(path, "REPORT_KNN_" + file_name + ".txt"), "w")
        ...
        n_neighbors = list(range(1,30))     # Valor de k
        p=[1,2]                             # Distancia: Manhattan o Euclidea
        ...
        file.write('Best p:' + str(best_model.best_estimator_.get_params()['p']) + "\n")
        file.write('Best n_neighbors:' + str(best_model.best_estimator_.get_params()['n_neighbors']) + "\n")

        # Predictions
        best_estimator = best_model.best_estimator_
        y_pred_train = cross_val_predict(best_estimator, X_train, y_train, cv=10)
        y_pred_test = best_estimator.predict(X_test)

        # Save the model
        joblib.dump(best_estimator, os.path.join(path, "KNN_MODEL_" + file_name + ".joblib"), compress=3)
        print('Trained model saved in: ' + os.path.join(path, os.path.join(path, "KNN_MODEL_" + file_name + ".joblib")))
        ...
        fig, ax = plt.subplots()
        knn_roc = KNeighborsClassifier(p=best_model.best_estimator_.get_params()['p'],
                                       n_neighbors=best_model.best_estimator_.get_params()['n_neighbors'])
        for i, (train, test) in enumerate(cv.split(X_train, y_train)):
            knn_roc.fit(X_train.iloc[train], y_train.iloc[train])
            viz = plot_roc_curve(knn_roc, X_train.iloc[test], y_train.iloc[test],
        ...
        print('Model evaluation report saved in: ' + os.path.join(path, "REPORT_KNN_" + file_name + ".txt"))
