...
class MLP(object):
    ...
    def train(train_dev, test, file_name, path):
        file = open(os.path.join(path, "REPORT_MLP_" + file_name + ".txt"), "w")
        ...
        # Use GridSearch
        clf = GridSearchCV(mlp, hyperparameters, cv=5)
        # Fit the model
        best_model = clf.fit(X_train, y_train)
        # Print The value of best Hyperparameters
        file.write('Best hidden_layer_sizes:' + str(best_model.best_estimator_.get_params()['hidden_layer_sizes']) + "\n")
        file.write('Best activation:' + str(best_model.best_estimator_.get_params()['activation']) + "\n")
        file.write('Best solver:' + str(best_model.best_estimator_.get_params()['solver']) + "\n")
        file.write('Best alpha:' + str(best_model.best_estimator_.get_params()['alpha']) + "\n")
        file.write('Best learning_rate:' + str(best_model.best_estimator_.get_params()['learning_rate']) + "\n")
        file.write('Best learning_rate_init:' + str(best_model.best_estimator_.get_params()['learning_rate_init']) + "\n")
        ...
        # Save the model
        joblib.dump(best_estimator, os.path.join(path, "MLP_MODEL_" + file_name + ".joblib"), compress=3)
        print('Trained model saved in: ' + os.path.join(path, os.path.join(path, "MLP_MODEL_" + file_name + ".joblib")))
        ...
        fig, ax = plt.subplots()
        mlp_roc = MLPClassifier(max_iter=100,
                                ...
                                learning_rate_init=best_model.best_estimator_.get_params()['learning_rate_init'])
        for i, (train, test) in enumerate(cv.split(X_train, y_train)):
            mlp_roc.fit(X_train.iloc[train], y_train.iloc[train])
            viz = plot_roc_curve(mlp_roc, X_train.iloc[test], y_train.iloc[test],
        ...
        print('Model evaluation report saved in: ' + os.path.join(path, "REPORT_MLP_" + file_name + ".txt"))
