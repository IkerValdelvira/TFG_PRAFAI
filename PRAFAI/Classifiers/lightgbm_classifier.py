...
class LightGBM(object):
    ...
    def train(train_dev, test, file_name, path):
        file = open(os.path.join(path, "REPORT_LightGBM_" + file_name + ".txt"), "w")
        ...
        hyperparameters = { 'max_depth': [3, 6, 10],
                            'num_leaves': [5, 10, 50],
        ...
        file.write('Best num_leaves:' + str(best_model.best_estimator_.get_params()['num_leaves']) + "\n")
        file.write('Best learning_rate:' + str(best_model.best_estimator_.get_params()['learning_rate']) + "\n")
        file.write('Best min_child_samples:' + str(best_model.best_estimator_.get_params()['min_child_samples']) + "\n")
        ...
        # Save the model
        joblib.dump(best_estimator, os.path.join(path, "LGBM_MODEL_" + file_name + ".joblib"), compress=3)
        print('Trained model saved in: ' + os.path.join(path, os.path.join(path, "LGBM_MODEL_" + file_name + ".joblib")))
        ...
        fig, ax = plt.subplots()
        classifier_roc = lgb.LGBMClassifier(max_depth=best_model.best_estimator_.get_params()['max_depth'],
                                        num_leaves=best_model.best_estimator_.get_params()['num_leaves'],
                                        learning_rate=best_model.best_estimator_.get_params()['learning_rate'],
                                        min_child_samples=best_model.best_estimator_.get_params()['min_child_samples'],
        ...
        plt.plot(fpr, tpr, color='red',
                 label='LightGBM (AUC = %0.2f)' % auc_metric)
        ...
        print('Model evaluation report saved in: ' + os.path.join(path, "REPORT_LightGBM_" + file_name + ".txt"))
    ...
    def evaluation_comparison(results, path):  # Por cada elemento en results obtenemos un array: [y_train, fpr, tpr, auc, classifier, X_test, y_test]
        ...
        if (len([val for val in results.keys() if 'StandardScaler' in val]) > 0):
            ...
            # plot roc curves
            if (len(result_mv_SS) != 0):
                plt.plot(fpr_mv, tpr_mv, color='yellow',
                         label=algorithm_name + ', Missing Values (AUC = %0.2f)' % auc_mv)
            if (len(result_mean_SS) != 0):
        ...
        if (len([val for val in results.keys() if 'QuantileTransformer' in val]) > 0):
            ...
            plt.legend(loc='best')
            plt.savefig(
                os.path.join(path, "QuantileTransformer", "Comparisons",
