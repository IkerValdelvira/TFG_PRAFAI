...
class SVM(object):
    ...
    def train(train_dev, test, file_name, path):
        file = open(os.path.join(path, "REPORT_SVM_" + file_name + ".txt"), "w")
        ...
        file.write('Best C:' + str(best_model.best_estimator_.get_params()['C']) + "\n")
        file.write('Best gamma:' + str(best_model.best_estimator_.get_params()['gamma']) + "\n")
        file.write('Best kernel:' + str(best_model.best_estimator_.get_params()['kernel']) + "\n")
        if(best_model.best_estimator_.get_params()['kernel'] == 'poly'):
            file.write('Best degree:' + str(best_model.best_estimator_.get_params()['degree']) + "\n")
        ...
        # Save the model
        joblib.dump(best_estimator, os.path.join(path, "SVM_MODEL_" + file_name + ".joblib"), compress=3)
        print('Trained model saved in: ' + os.path.join(path, os.path.join(path, "SVM_MODEL_" + file_name + ".joblib")))
        ...
        svm_roc = SVC(probability=True,
                  ...
                  kernel=best_model.best_estimator_.get_params()['kernel'],
                  degree=best_model.best_estimator_.get_params()['degree'])
        for i, (train, test) in enumerate(cv.split(X_train, y_train)):
            svm_roc.fit(X_train.iloc[train], y_train.iloc[train])
            viz = plot_roc_curve(svm_roc, X_train.iloc[test], y_train.iloc[test],
        ...
        print('Model evaluation report saved in: ' + os.path.join(path, "REPORT_SVM_" + file_name + ".txt"))
