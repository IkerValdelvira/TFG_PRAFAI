...
class FCN_ENC(object):
    ...
    def model(ecg_dict, labels, output_dir):
        ...
        FCN_ENC.train_eval(object, ids, np.array(ecg_array), np.array(labels), output_dir)
    ...
    def train_eval(self, ids, features, labels, output_dir):
        ...
        joblib.dump(scaler, os.path.join(output_dir, 'std_scaler_FCN_Encoder.bin'), compress=True)
        print("Standard scaler saved in: " + str(os.path.join(output_dir, 'std_scaler_FCN_Encoder.bin')))
        ...
        fcn_encoder_model = keras.models.Model(inputs=inputA, outputs=final_layer)

        fcn_encoder_model.compile(loss=tf.keras.losses.BinaryCrossentropy(), optimizer=tf.keras.optimizers.Adam(learning_rate=0.001),
                          metrics=[tf.keras.metrics.BinaryAccuracy(name='accuracy'),
                                   tf.keras.metrics.Recall(name='recall'),
                                   tf.keras.metrics.Precision(name='precision'),
                                   tf.keras.metrics.AUC(name="AUC")])

        file = open(os.path.join(output_dir, "REPORT_FCN_Encoder" + ".txt"), "w")
        file.write("FCN+Encoder model: 12-Lead ECG classification\n\n")
        fcn_encoder_model.summary(print_fn=lambda x: file.write(x + '\n'))

        csv_logger = CSVLogger(os.path.join(output_dir, 'train_log_FCN_Encoder.csv'), append=True, separator=';')
        ...
        print("\nTraining FCN_Encoder model...")
        train = fcn_encoder_model.fit(X_train, y_train, batch_size=batch_size, epochs=epochs, verbose=1, validation_data=(X_val, y_val), callbacks=[csv_logger,reduce_lr,early_stop])

        fcn_encoder_model.save(os.path.join(output_dir, "fcn_encoder_model.h5"))
        print('Trained model saved in: ' + os.path.join(output_dir, "fcn_encoder_model.h5"))

        print('\nEvaluating model...')

        # DEV evaluation
        print('Evaluating on Dev set...')
        dev_eval = fcn_encoder_model.evaluate(X_dev, y_dev, verbose=1)
        ...
        predicted_classes_dev = np.where(fcn_encoder_model.predict(X_dev) > 0.5, 1, 0)
        ...
        # TEST evaluation
        test_eval = fcn_encoder_model.evaluate(X_test, y_test, verbose=1)
        ...
        predicted_classes_test = np.where(fcn_encoder_model.predict(X_test) > 0.5, 1, 0)
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, "ConfusionMatrix_FCN_Encoder.png"))
        print('Confusion matrices saved in: ' + os.path.join(output_dir, "ConfusionMatrix_FCN_Encoder.png"))
        plt.close()

        # Test predictions probabilities
        y_pred = np.where(fcn_encoder_model.predict(X_test) > 0.5, 1, 0)
        i = 0
        results = []
        for y in y_test:
        ...
        y_pred_keras = fcn_encoder_model.predict(X_dev).ravel()
        ...
        plt.plot(fpr_keras, tpr_keras, color='red',
                 label='FCN_Encoder (AUC = %0.4f)' % auc_keras)
        ...
        plt.legend(loc='best')
        plt.savefig(os.path.join(output_dir, "ROCcurve(Dev)_FCN_Encoder.png"))
        print('Dev ROC curve and AUC saved in: ' + os.path.join(output_dir, "ROCcurve(Dev)_FCN_Encoder.png"))
        plt.close()

        # ROC curve and AUC (Test)
        # roc curve for models
        y_pred_keras = fcn_encoder_model.predict(X_test).ravel()
        ...
        plt.ylabel('True Positive rate')
        plt.legend(loc='best')
        plt.savefig(os.path.join(output_dir, "ROCcurve(Test)_FCN_Encoder.png"))
        print('Test ROC curve and AUC saved in: ' + os.path.join(output_dir, "ROCcurve(Test)_FCN_Encoder.png"))
        plt.close()

        print('Model evaluation report saved in: ' + os.path.join(output_dir, "REPORT_FCN_Encoder.txt"))
