...
class LSTM(object):
    ...
    def model(ecg_dict, labels, output_dir):
        ...
        LSTM.train_eval(object, ids, np.array(ecg_array), np.array(labels), output_dir)
    ...
    def train_eval(self, ids, features, labels, output_dir):
        ...
        joblib.dump(scaler, os.path.join(output_dir, 'std_scaler_RNN.bin'), compress=True)
        print("Standard scaler saved in: " + str(os.path.join(output_dir, 'std_scaler_RNN.bin')))
        ...
        rnn_model.add(keras.layers.SimpleRNN(128, input_shape=(5000,12), return_sequences=True))
        rnn_model.add(keras.layers.Dropout(0.2))
        rnn_model.add(keras.layers.SimpleRNN(256, return_sequences=True))
        rnn_model.add(keras.layers.Dropout(0.2))
        rnn_model.add(keras.layers.SimpleRNN(100))
        rnn_model.add(keras.layers.Dropout(0.2))
        rnn_model.add(keras.layers.Dense(1))"""

        # GRU
        """rnn_model = keras.Sequential()
        rnn_model.add(keras.layers.GRU(64, input_shape=(5000,12), return_sequences=True))
        rnn_model.add(keras.layers.Dropout(0.2))
        rnn_model.add(keras.layers.GRU(128, return_sequences=True))
        rnn_model.add(keras.layers.Dropout(0.2))
        rnn_model.add(keras.layers.GRU(100))
        rnn_model.add(keras.layers.Dropout(0.2))
        rnn_model.add(keras.layers.Dense(1, activation='linear'))"""

        # LSTM
        """rnn_model = keras.Sequential()
        rnn_model.add(keras.layers.LSTM(64, input_shape=(5000,12), return_sequences=True))
        rnn_model.add(keras.layers.Dropout(0.2))
        ...
        rnn_model.compile(loss=tf.keras.losses.MeanSquaredError(), optimizer=tf.keras.optimizers.Adam(learning_rate=0.001),
        ...
        lstm_model.add(keras.layers.Bidirectional(keras.layers.LSTM(100, return_sequences=True), input_shape=(5000,12)))
        lstm_model.add(keras.layers.Dropout(0.5))
        lstm_model.add(keras.layers.Bidirectional(keras.layers.LSTM(100)))
        lstm_model.add(keras.layers.Dropout(0.5))
        lstm_model.add(keras.layers.Dense(1, activation='sigmoid'))

        lstm_model.compile(loss=tf.keras.losses.BinaryCrossentropy(), optimizer=tf.keras.optimizers.Adam(learning_rate=0.001),
                          metrics=[tf.keras.metrics.BinaryAccuracy(name='accuracy'),
                                   tf.keras.metrics.Recall(name='recall'),
                                   tf.keras.metrics.Precision(name='precision'),
                                   tf.keras.metrics.AUC(name="AUC")])

        file = open(os.path.join(output_dir, "REPORT_LSTM" + ".txt"), "w")
        file.write("LSTM model: 12-Lead ECG classification\n\n")
        lstm_model.summary(print_fn=lambda x: file.write(x + '\n'))

        csv_logger = CSVLogger(os.path.join(output_dir, 'train_log_LSTM.csv'), append=True, separator=';')
        ...
        print("\nTraining LSTM model...")
        train = lstm_model.fit(X_train, y_train, batch_size=batch_size, epochs=epochs, verbose=1, validation_data=(X_val, y_val), callbacks=[csv_logger,reduce_lr,early_stop])

        lstm_model.save(os.path.join(output_dir, "lstm_model.h5"))
        print('Trained model saved in: ' + os.path.join(output_dir, "lstm_model.h5"))

        print('\nEvaluating model...')

        # DEV evaluation
        print('Evaluating on Dev set...')
        dev_eval = lstm_model.evaluate(X_dev, y_dev, verbose=1)
        ...
        predicted_classes_dev = np.where(lstm_model.predict(X_dev) > 0.5, 1, 0)
        ...
        test_eval = lstm_model.evaluate(X_test, y_test, verbose=1)
        ...
        predicted_classes_test = np.where(lstm_model.predict(X_test) > 0.5, 1, 0)
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, "ConfusionMatrix_LSTM.png"))
        print('Confusion matrices saved in: ' + os.path.join(output_dir, "ConfusionMatrix_LSTM.png"))
        plt.close()
        ...
        y_pred_keras = lstm_model.predict(X_dev).ravel()
        ...
        plt.legend(loc='best')
        plt.savefig(os.path.join(output_dir, "ROCcurve(Dev)_LSTM.png"))
        print('Dev ROC curve and AUC saved in: ' + os.path.join(output_dir, "ROCcurve(Dev)_LSTM.png"))
        plt.close()
        ...
        plt.ylabel('True Positive rate')
        plt.legend(loc='best')
        plt.savefig(os.path.join(output_dir, "ROCcurve(Test)_LSTM.png"))
        print('Test ROC curve and AUC saved in: ' + os.path.join(output_dir, "ROCcurve(Test)_LSTM.png"))
        plt.close()

        print('Model evaluation report saved in: ' + os.path.join(output_dir, "REPORT_LSTM.txt"))
