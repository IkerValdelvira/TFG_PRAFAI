...
class FCN(object):
    ...
    def model(ecg_dict, labels, output_dir):
        ...
        FCN.train_eval(object, ids, np.array(ecg_array), np.array(labels), output_dir)
    ...
    def train_eval(self, ids, features, labels, output_dir):
        ...
        joblib.dump(scaler, os.path.join(output_dir, 'std_scaler_FCN.bin'), compress=True)
        print("Standard scaler saved in: " + str(os.path.join(output_dir, 'std_scaler_FCN.bin')))
        ...
        conv1 = keras.layers.Conv1D(filters=128, kernel_size=8, input_shape=(5000, 12), padding='same')(input_layer)
        ...
        outputlayer = keras.layers.Dense(1, activation='sigmoid')(gap_layer)

        fcn_model = keras.Model(inputs=input_layer, outputs=outputlayer)

        fcn_model.compile(loss=tf.keras.losses.BinaryCrossentropy(), optimizer=tf.keras.optimizers.Adam(learning_rate=0.001),
                          metrics=[tf.keras.metrics.BinaryAccuracy(name='accuracy'),
                                   tf.keras.metrics.Recall(name='recall'),
                                   tf.keras.metrics.Precision(name='precision'),
                                   tf.keras.metrics.AUC(name="AUC")])

        file = open(os.path.join(output_dir, "REPORT_FCN" + ".txt"), "w")
        file.write("FCN model: 12-Lead ECG classification\n\n")
        fcn_model.summary(print_fn=lambda x: file.write(x + '\n'))

        csv_logger = CSVLogger(os.path.join(output_dir, 'train_log_FCN.csv'), append=True, separator=';')
        ...
        print("\nTraining FCN model...")
        train = fcn_model.fit(X_train, y_train, batch_size=batch_size, epochs=epochs, verbose=1, validation_data=(X_val, y_val), callbacks=[csv_logger,reduce_lr,early_stop])

        fcn_model.save(os.path.join(output_dir, "fcn_model.h5"))
        print('Trained model saved in: ' + os.path.join(output_dir, "fcn_model.h5"))

        print('\nEvaluating model...')

        # DEV evaluation
        print('Evaluating on Dev set...')
        dev_eval = fcn_model.evaluate(X_dev, y_dev, verbose=1)
        ...
        predicted_classes_dev = np.where(fcn_model.predict(X_dev) > 0.5, 1, 0)
        ...
        test_eval = fcn_model.evaluate(X_test, y_test, verbose=1)
        ...
        predicted_classes_test = np.where(fcn_model.predict(X_test) > 0.5, 1, 0)
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, "ConfusionMatrix_FCN.png"))
        print('Confusion matrices saved in: ' + os.path.join(output_dir, "ConfusionMatrix_FCN.png"))
        plt.close()

        # Test predictions probabilities
        y_pred = np.where(fcn_model.predict(X_test) > 0.5, 1, 0)
        i = 0
        results = []
        for y in y_test:
        ...
        y_pred_keras = fcn_model.predict(X_dev).ravel()
        ...
        plt.legend(loc='best')
        plt.savefig(os.path.join(output_dir, "ROCcurve(Dev)_FCN.png"))
        print('Dev ROC curve and AUC saved in: ' + os.path.join(output_dir, "ROCcurve(Dev)_FCN.png"))
        plt.close()

        # ROC curve and AUC (Test)
        # roc curve for models
        y_pred_keras = fcn_model.predict(X_test).ravel()
        ...
        plt.ylabel('True Positive rate')
        plt.legend(loc='best')
        plt.savefig(os.path.join(output_dir, "ROCcurve(Test)_FCN.png"))
        print('Test ROC curve and AUC saved in: ' + os.path.join(output_dir, "ROCcurve(Test)_FCN.png"))
        plt.close()

        print('Model evaluation report saved in: ' + os.path.join(output_dir, "REPORT_FCN.txt"))
